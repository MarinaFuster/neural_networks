...
def extract_wininst_cfg(dist_filename):
    """Extract configuration files from a bdist_wininst .exe
    ...
    try:
        ...
        if prepended < 12:  # no wininst files here
            return None
        f.seek(prepended - 12)
...
def update_dist_caches(dist_path, fix_zipimporter_caches):
    """
    Fix any globally cached `dist_path` related files

    `dist_path` should be a path of a newly installed egg distribution (zipped
    or unzipped).

    sys.path_importer_cache contains finder objects that have been cached when
    importing files from the original distribution. Any such finders need to be
    ...
    files packaged inside zip archives. If stale loaders referencing the
    original distribution, are left behind, they can fail to load modules from
    the replacement distribution. E.g. if an old zipimport.zipimporter instance
    is used to load files from a new zipped egg archive, it may cause the
    operation to attempt to locate the requested files in the wrong location -
    ...
    our best to clear any remaining zipimport.zipimporter related cached files
    that might somehow later get used when attempting to load files from the new
    distribution and thus cause such load operations to fail. Note that when
    tracking down such remaining stale files, we can not catch every conceivable
    ...
    if fix_zipimporter_caches:
        ...
        # This whole stale files removal step does not seem strictly necessary,
...
def _update_zipimporter_cache(normalized_path, cache, updater=None):
    """
    Update zipimporter cache files for a given normalized path.
